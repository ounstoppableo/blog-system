...
export class AddMsgFormComponent implements OnInit {
  ...
  addMsg() {
    ...
    if (this.isMsgBoard) {
      this.boardMsgService.addMsgForBoard(data).subscribe((res: resType<any>) => {
        if (res.code === 200) {
      ...
      this.boardMsgService.addMsgForArticle(data).subscribe((res: resType<any>) => {
        if (res.code === 200) {
          this.message.success('评论成功')
          this.msgBoardData.reset()
          this.reloadMsg.emit()
  ...
  constructor(private message: NzMessageService, private boardMsgService: BoardMsgService, private route: ActivatedRoute) { }
}
