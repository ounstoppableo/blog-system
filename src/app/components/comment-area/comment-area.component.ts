...
export class CommentAreaComponent implements OnInit {
  ...
  limit = 5
  total = 0
  constructor(private addMsgService: BoardMsgService, private route: ActivatedRoute) { }
  ngOnInit(): void {
  ...
  getMsgItems(page: number, limit: number) {
    ...
    this.loading = true
    if (this.isMsgBoard) {
      this.addMsgService.getMsgForBoard(page, limit).subscribe((res: resType<any>) => {
        if (res.code === 200) {
      ...
      this.addMsgService.getMsgForArticle(this.articleId, page, limit).subscribe((res: resType<any>) => {
        if (res.code === 200) {
          this.msgItems = res.data.msgData as msgItem[]
          this.msgCount = res.data.msgCount
          this.total = res.data.pages * limit
